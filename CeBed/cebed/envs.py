...
class EnvConfig:
    ...
    n_ues: int = 1
    # Carrier frequency
    carrier_frequency: int = 2100000000.0
...
class OfdmEnv:
    ...
    def estimate_at_pilot_locations(self, y: tf.Tensor) -> tf.Tensor:
        ...
        # pdb.set_trace()
        # print(f"h_ls shape is: {h_ls.shape}")
        # Build pilot masks for every stream
        pilot_mask = self._build_pilot_mask()

        # Build indices for mapping channel estimates
        ...
        _inputs_to_rg_indices = tf.cast(inputs_to_rg_indices, tf.int32)   #changes the data type
        ...
        # Expand rank of pilots for broadcasting or we can say we need h_hat has the similar dimension to resource grid
        h_hat = tf.scatter_nd(
