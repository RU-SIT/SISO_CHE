...
from cebed.utils import tfinterpolate  #Resize a complex tensor to new size using the specified interpolation method.
        ...
        for nt in range(n_tx):
            for nr in range(n_rx_ant):
                for k, i in enumerate(pilot_locations):
                    ...
                    rhp = np.matmul(hp, hp.conj().T)    #hp: noise free ls channel
                    rhhp = np.matmul(hi, hp.conj().T)   #hi: channel(ground truth)
                    pinv = np.linalg.pinv(              #Moore-Penrose pseudoinverse of a matrix using singular value decomposition
