...
class TrainConfig:
    ...
    seed: int = 42
    verbose: int = 1
    output_dir: str = "for_testing_changes"   # change the name of output dir based on the experiment as follow: "ChannelNet_f{numtrainingdata}"
...
class Trainer:
    ...
    def setup(self):
        ...
        self.dataset = dataset_class(
            ...
            main_train_split=self.config.main_train_split,
        ...
        # Print dataset sizes
        train_size = sum(1 for _ in self.train_loader)
        val_size = sum(1 for _ in self.eval_loader)
        test_size = len(self.dataset.test_indices[0])  

        print(f"Training set size: {train_size * self.config.train_batch_size}")
        print(f"Validation set size: {val_size * self.config.eval_batch_size}")
        print(f"Test set size: {test_size}")

        # Create model
        model_hparams = cm.get_model_hparams(
            self.config.model_name, self.config.experiment_name
        )
    ...
    def evaluate_ls(
        ...
        h_ls = tf.math.divide_no_nan(y_pilot_noisy, self.dataset.pilots)  #h_ls = y_pilot_noisy / TX_at_pilot_
    ...
    def evaluate(
        ...
        # this apply_noiseless_channel variable is used to produce noiseless recived signals 
        apply_noiseless_channel = ApplyOFDMChannel(
            add_awgn=False, dtype=tf.as_dtype(tf.complex64)
        )
        # this apply_noisy_channel variable is used to produce noisy recived signals if it is not available in the data
        ...
        pdb.set_trace()
        print("Test_indecies:{}".format(self.dataset.test_indices[0]))
        for i, snr in enumerate(tqdm(snr_range)):
            ...
            for step in range(num_steps):
                ...
                if self.dataset.x_samples is None:
                    ...
                    symbols = self.dataset.x_samples[i][self.dataset.test_indices[i]][     #transmitter signal
                        start:end
                    ]
                # ['h'] keys in the data dictionary look for MultiDomainOfflineSionnaDataset in projects/MyPrivaterepo/CeBed/cebed/datasets/sionna.py
                channels = self.dataset.test_y[i][start:end]   
                pdb.set_trace()
                
                # plot noisy_signals (here is recieved signal)
                if self.dataset.y_samples is None:
                    noisy_signals = apply_noisy_channel([symbols, channels, noise_lin])    #recieved signal
                else:
                    noisy_signals = self.dataset.y_samples[i][        #recieved signal
                        self.dataset.test_indices[i]
                    ][start:end]
                # plot noiseless_signals
                noiseless_signals = apply_noiseless_channel([symbols, channels])  #noiseless recived signal

                # evaluate the NN model
                inputs = self.dataset.env.estimate_at_pilot_locations(noisy_signals) #look for estimate_at_pilot_locations() at projects/MyPrivaterepo/CeBed/cebed/envs.py 
                # pdb.set_trace()
                pre_inputs = tf.map_fn(
                    self.dataset.preprocess_inputs,   #reshaping, masking, converting to real
                    inputs,
                    fn_output_signature=tf.float32,
                )
                h_hat = self.model(pre_inputs, training=False)
                h_hat_n = tf.map_fn(
                    postprocess, h_hat, fn_output_signature=tf.complex64    #real to complex and reshaping
                )
    ...
    def inspect_data(self):
        ...
        for inputs, labels in self.train_loader.take(1):
