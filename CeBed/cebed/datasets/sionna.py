...
class OfflineSionnaDataset(TensorOfflineDataset):
    ...
    def __init__(
        self,
        path: str,
        main_train_split: float = 0.9,  #to have unchnged test set
        ...
        super().__init__(
            path=path, seed=seed, train_split=train_split, main_train_split=main_train_split, input_type=input_type
        )
...
class MultiDomainOfflineSionnaDataset(OfflineSionnaDataset):
    ...
    def __init__(
        ...
        path: str,
        main_train_split: float = 0.9,  #to have unchnged test set
        train_split: float = 0.8, # Here this needs to be change, add arg to run for different veriant variant of training splitting
        input_type: str = "low",
        seed: int = 0,
    ):
        ...
        self.main_train_split = main_train_split  #to have unchnged test set
    ...
    def split(self):
        ...
        for ds in range(self.num_domains):
            ...
            train_indices, test_indices = make_split(
                len(self.inputs[ds]), train_fraction=self.main_train_split
            )
