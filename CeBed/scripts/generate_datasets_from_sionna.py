...
# def get_data(env: OfdmEnv, args: Dict[str, Any]):
#     """Yield data for each domain"""
#     if args.num_domains == 1:
#         yield args.start_ds, generate_samples_from_env(env, args.start_ds, args.size, args.batch_size)
#     else:
#         step = int((args.end_ds - args.start_ds) / args.num_domains)
#         for snr_db in trange(args.start_ds, args.end_ds, step):
#             task_data = generate_samples_from_env(env, snr_db, args.size, args.batch_size)
#             yield snr_db, task_data
...
#     # build your env config and env exactly as before
...
#     # compute the list of SNRs/domains exactly as get_data did
#     if args.num_domains > 1:
#         step = int((args.end_ds - args.start_ds) / args.num_domains)
#         snr_list = list(range(args.start_ds, args.end_ds, step))
#     else:
#         snr_list = [args.start_ds]

#     for snr_db in snr_list:
#         # generate one domain’s worth of data
#         domain_data = generate_samples_from_env(env, snr_db, args.size, args.batch_size)

#         # write it to its own file
#         fname = os.path.join(out_dir, f"data_snr{snr_db}.hdf5")
...
#                     chunks=True,
#                 )
#         print(f"  → Saved domain SNR={snr_db} to {fname}")

#     # metadata is the same for all domains, so write once
#     write_metadata(os.path.join(out_dir, "metadata.yaml"), env_config)

#     elapsed = time.time() - start_time
#     print(f"Finished data generation in {elapsed:.2f}s")
...
if __name__ == "__main__":
    ...
    from paths import default_cebed_output

    parser = argparse.ArgumentParser("Data Generation")
    parser.add_argument("--output_dir", type=str, default=default_cebed_output())
    parser.add_argument("--size", type=int, default= 10000)
    parser.add_argument("--batch_size", type=int, default= 100)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--num_domains", type=int, default=5, help="Number of domains/tasks generated"
    )
    ...
    parser.add_argument(
        "-nf", "--fft_size", type=int, default=612, help="Number of subcarriers"
    )
    ...
    parser.add_argument("--shadowing", default="True", help="Shadowing")
