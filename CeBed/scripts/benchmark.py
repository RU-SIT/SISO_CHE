...
INPUT_TYPES = {
    ...
    "DDAE": "raw",
    "HA02": "low",
    "MTRE": "raw",         #"raw": returns the frame as it is.    
    "ChannelNet": "low",   #"low": applies a mask to the frame and reduces its dimensions
}
...
parser.add_argument("--output_dir", type=str, default="train_output_1precent_training")
########################## Training params ################################
...
parser.add_argument("--train_split", type=float, default=0.005)
parser.add_argument(
...
if "ChannelNet" in MODELS:
    cmd = (
        f"CUDA_VISIBLE_DEVICES={preferred_gpu} python scripts/train.py --experiment_name {args.experiment_name} --seed {args.seed} --output_dir {args.output_dir} "
        f"--data_dir {args.data_dir} -v {args.verbose} --lr {args.lr} "
        f"--train_batch_size {args.train_batch_size} --eval_batch_size {args.eval_batch_size} --train_split {args.train_split} --epochs {args.epochs} "
        f"--dataset_name {args.dataset_name} --model_name ChannelNet --input_type {INPUT_TYPES['ChannelNet']}"
    )
    commands.append(cmd)
...
# parser.add_argument("--output_dir", type=str, default="train_output_60precent_training")
# ########################## Training params ################################
# parser.add_argument("--epochs", type=int, default=100, help="Number of training epochs")
# parser.add_argument("--train_batch_size", type=int, default=32)
# parser.add_argument("--eval_batch_size", type=int, default=32)
