...
class Meta(nn.Module):
    ...
    def forward(self, x_qry, y_qry, x_spt, y_spt):
        ...
        # Average loss - need to check if this should be / batchsz or / n_way
        # losses accumulate over n_way iterations
        loss_q = losses_s[-1] / self.n_way

        self.meta_optim.zero_grad()
        loss_q.backward() 
        
        # Gradient clipping to stabilize meta-learning (helps with high variance in MAML)
        # Clips gradients to prevent explosion, especially useful when:
        # 1. Loss variance is high (like your case)
        # 2. Multiple tasks create conflicting gradients
