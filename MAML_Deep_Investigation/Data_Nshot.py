...
class ChannelEstimationNShot:
    def __init__(self, root, batchsz, n_way, k_shot, k_query):
        ...
        if self.train_files_by_snr:
            print("SNR-based task groups available for training:")
            for snr in sorted(self.train_files_by_snr.keys()):
                print(f"  {snr}dB SNR: {len(self.train_files_by_snr[snr])} channels")
                print(f"    Channels: {', '.join([f.split('_')[-1].replace('.mat', '') for f in self.train_files_by_snr[snr]])}")
    ...
    def load_data_cache(self, mode):
        ...
        for _ in range(self.batchsz if mode == 'train' else 1):  # One task during fine-tuning
            ...
            for _ in range(1 if mode == 'test' else self.n_way):  # One task in testing mode
                ...
                # This creates 3 additional task types: 0db SNR group, 5db SNR group, 10db SNR group
                if mode == 'train' and files_by_snr:
                    ...
                    if use_snr_grouping:
                        ...
                        if available_snrs:
                            ...
                            # Sample from files with the selected SNR (with replacement if needed)
                            selected_files = np.random.choice(snr_file_pool, self.n_way, replace=True)
                        else:
                            # Fallback to random sampling if no SNR groups available
                            selected_files = np.random.choice(file_names, self.n_way, replace=True)
                    else:
                        # Random sampling (original behavior)
                        selected_files = np.random.choice(file_names, self.n_way, replace=True)
                else:
                    # For test mode or if no SNR grouping available, use random sampling
