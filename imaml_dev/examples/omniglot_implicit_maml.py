...
from paths import imaml_examples_dir
...
parser.add_argument('--load_agent', type=str, default=None)  # main model 
...
pdb.set_trace()
losses = np.zeros((args.meta_steps, 4)) #(500, 4)
accuracy = np.zeros((args.meta_steps, 2)) #(500, 2)
for outstep in tqdm(range(args.meta_steps)):   #line 2 of algorithm 1 
    task_mb = np.random.choice(args.num_tasks, size=args.task_mb_size)  #(32,) #sample minibatch of tasks, line 3 of alorithm 1
    w_k = meta_learner.get_params() #torch.Size([113221])
    meta_grad = 0.0
    lam_grad = 0.0
    # pdb.set_trace()
    for idx in task_mb:    # start compute each tak meta gard (line 4 of the algorithm 1) 
        fast_learner.set_params(w_k.clone()) # sync weights
        pdb.set_trace()
        task = dataset.__getitem__(idx) #dict_keys(['task_def', 'x_train', 'y_train', 'x_val', 'y_val'])
        vl_before = fast_learner.get_loss(task['x_val'], task['y_val'], return_numpy=True)   
        tl = fast_learner.learn_task(task, num_steps=args.n_steps) #alg_i len(tl) = 16
        # pull back for regularization
        fast_learner.inner_opt.zero_grad()
        regu_loss = fast_learner.regularization_loss(w_k, lam) # implicit meta-grad computation(alg2)
        regu_loss.backward()
        ...
        #compute partial outer level grad v_i (line 4 algorithm 2)
        valid_loss = fast_learner.get_loss(task['x_val'], task['y_val']) 
        valid_grad = torch.autograd.grad(valid_loss, fast_learner.model.parameters()) #len(valid_grad) = 18
        flat_grad = torch.cat([g.contiguous().view(-1) for g in valid_grad]) # flat_grad.shape = torch.Size([113221]) #The contiguous() is used to ensure that a tensor's data is stored in a contiguous block of memory
        
        #iterative solver(CG) along with hessian vector products to compute gi(line 5 of algorithm 2):
        # norm (gi - (I + (lam/2 hessian * v_i)))
        if args.cg_steps <= 1:
            task_outer_grad = flat_grad
        else:
            task_matrix_evaluator = fast_learner.matrix_evaluator(task, lam, args.cg_damping)
            
            task_outer_grad = utils.cg_solve(task_matrix_evaluator, flat_grad, args.cg_steps, x_init=None)  #task_outer_grad.shape = torch.Size([113221])
        
        meta_grad += (task_outer_grad/args.task_mb_size) #torch.Size([113221]), # line 7 of algorithm 1 
    ...
    meta_learner.outer_step_with_grad(meta_grad, flat_grad=True)  # update meta param (line 8 of algorithm 2)
